...
This can be useful when we read in other people's data with long column names, for example.  
...
ggplot(data = squads, aes(x = player, y = goals)) +
  geom_col() +
  coord_flip()  # Makes a bar plot with ggplot, but we can't see anything in there are there are so many players!
...
In the ```wwc_outcomes dataframe```, we have the teams, but we might not know what all the short versions mean. However, they are stored in ```codes```:
...
The function ```left_join``` retains the dataframe that we specify first, and adds in values from the dataframe we specify next. There are different ```join``` functions and they do differnt things.
...
R orders factors alphabetically. Often we do not want to display bar plots in tjhis way, but by a frequency or another value. For example, we made the following plot earlier:
...
We want to order the plot by gpc really. This is where ```fct_reorder()``` comes in:
